"use client";
import React from "react";
import Image from "next/image";
import { Button } from "@/components/ui/button";
import { useLocale } from "@/lib/i18n/locale-context";

const Hero = () => {
  const { t } = useLocale();
  return (
    <div className="relative h-screen w-full flex items-center justify-center text-white-100">
      <Image
        src="/images/hero-background.avif"
        alt="Hero Image"
        fill={true}
        style={{ objectFit: "cover" }}
        className="absolute inset-0 "
        sizes="100vw"
        priority
      />
      <div className="bg-img-overlay"></div>
      <div className="flex flex-col max-w-3xl text-center font-inter justify-center text-cream-200 items-center mx-auto px-1 mb-32 z-10">
        <small className="font-semibold text-xl text-wineRed-100">
          {t.hero.smallTitle}
        </small>
        <h1 className="text-6xl font-bold font-playfair text-white-200 mb-4">
          {t.hero.heading}
        </h1>
        <p className="text-xl max-w-xl text-cream-200">{t.hero.paragraph}</p>
        <div className="space-x-4">
          <Button
            variant="default"
            className="mt-4 bg-gold-100 text-white-100 hover:bg-wineRed-100 
        transition-colors duration-300"
          >
            {t.hero.cta1}
          </Button>
          <Button
            variant="outline"
            className="mt-4 bg-transparent text-white-100 hover:bg-wineRed-100 hover:text-cream-100 hover:border-wineRed-100
        transition-colors duration-300"
          >
            {t.hero.cta2}
          </Button>
        </div>
      </div>
    </div>
  );
};

export default Hero;
